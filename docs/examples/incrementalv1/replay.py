...
def main():   
    ...
    parser.add_argument('--data_source_0', type = str, default = 'austin')
    ...
    parser.add_argument('--test_split_coef_a', type = float, default = 0.3)
    ...
    train_visu = (args.model_path+"_results").format(args.training_mode,args.data_source_0,  args.data_target,args.seed)   
    ...
    early_stopping = EarlyStopping(patience=20, verbose=True, delta=0.001,path=args.model_path.format(args.training_mode,args.data_source_0, args.data_target, args.seed))
    print (args.data_source_1) 
    ...
    # Tiles Domain A
    a_domain_img = glob.glob(os.path.join(args.data_path, 'images/{}*.tif'.format(args.data_source_0)))
    a_domain_lbl = glob.glob(os.path.join(args.data_path, 'gt/{}*.tif'.format(args.data_source_0)))
    
    a_domain_img_train = a_domain_img[:int(len(a_domain_img)*0.075)]
    a_domain_lbl_train = a_domain_lbl[:int(len(a_domain_lbl)*0.075)]
       
    a_domain_img_inf = a_domain_img[int(len(a_domain_img)*0.075):]
    a_domain_lbl_inf = a_domain_lbl[int(len(a_domain_lbl)*0.075):]
    
    a_domain_img = glob.glob(os.path.join(args.data_path, 'images/{}*.tif'.format(args.data_source_1)))
    a_domain_lbl = glob.glob(os.path.join(args.data_path, 'gt/{}*.tif'.format(args.data_source_1)))
    ...
    a_domain_img = glob.glob(os.path.join(args.data_path, 'images/{}*.tif'.format(args.data_source_2)))
    a_domain_lbl = glob.glob(os.path.join(args.data_path, 'gt/{}*.tif'.format(args.data_source_2)))
    ...
    a_domain_img = glob.glob(os.path.join(args.data_path, 'images/{}*.tif'.format(args.data_source_3)))
    a_domain_lbl = glob.glob(os.path.join(args.data_path, 'gt/{}*.tif'.format(args.data_source_3)))
    ...
    a_domain_img_val = a_domain_img_inf[:int(len(a_domain_img_inf)*args.test_split_coef_a)]
    a_domain_lbl_val = a_domain_lbl_inf[:int(len(a_domain_lbl_inf)*args.test_split_coef_a)]
    
    a_domain_img_test = a_domain_img_inf[int(len(a_domain_img_inf)*args.test_split_coef_a):]
    a_domain_lbl_test = a_domain_lbl_inf[int(len(a_domain_lbl_inf)*args.test_split_coef_a):]
        
    train_domains_imgs = b_domain_img_train + a_domain_img_train
    train_domains_lbls = b_domain_lbl_train + a_domain_lbl_train
    train_image_paths_list, train_label_paths_list = shuffle(np.array(train_domains_imgs),np.array(train_domains_lbls))
    
    val_domains_imgs = b_domain_img_val  + a_domain_img_val 
    val_domains_lbls = b_domain_lbl_val  + a_domain_lbl_val 
    val_image_paths_list, val_label_paths_list = shuffle(np.array(val_domains_imgs),np.array(val_domains_lbls))
    
    # Test dataset
    test_list = open("test_{}.txt".format(train_visu), "w")
    for item in (a_domain_img_test,a_domain_lbl_test, b_domain_img_test,b_domain_lbl_test):
        test_list.write("%s\n" % item)
    test_list.close()
