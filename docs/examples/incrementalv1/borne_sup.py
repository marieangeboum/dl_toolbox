...
def main():   
    ...
    parser.add_argument('--data_source_list',type = str, nargs = '+', default = [])
    ...
    early_stopping = EarlyStopping(patience=20, verbose=True, delta=0.001,path=args.model_path.format(len(args.data_source_list),args.img_aug, args.seed))
    ...
    for data_source_1 in args.data_source_list : 
        # Tiles Domain A
        a_domain_img += glob.glob(os.path.join(args.data_path, 'images/{}*.tif').format(data_source_1))
        a_domain_lbl += glob.glob(os.path.join(args.data_path, 'gt/{}**.tif'.format(data_source_1)))
    ...
    test_list = open("test_{}.txt".format("all"), "w")
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        seg_img_visu.display_batch(train_writer, batch, 20,epoch,prefix='train')
