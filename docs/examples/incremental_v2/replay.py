...
def main():   
    ...
    parser.add_argument('--train_split_coef_b', type = float, default = 0.8)
    
    parser.add_argument('--tile_width', type = int, default = 5000)
    parser.add_argument('--tile_height', type = int, default = 5000)
    parser.add_argument('--model_path', type = str, default ="replay/models/s1/{}_{}_ft_{}_seed_{}")
    ...
    args.saved_model_path = 'replay/models/s1/{}_{}_ft_{}_seed_{}'.format(args.training_mode, len(args.source_list)-1, args.data_source,args.run)
    
    seed = int((args.saved_model_path).split('_')[-1])
    torch.manual_seed(seed)
    torch.cuda.manual_seed(seed)
    torch.autograd.set_detect_anomaly(True)
    #args.saved_model_path = 'models/kitsap_no_seed_{}'.format(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed(seed)
    
    torch.autograd.set_detect_anomaly(True) # stops training if something is wrong
    
    train_visu = (args.model_path+"_results").format(args.training_mode,len(args.source_list),  args.data_target,seed)   
    ...
    b_domain_img_val = b_domain_img[int(len(b_domain_img)*args.train_split_coef_b):]
    b_domain_lbl_val = b_domain_lbl[int(len(b_domain_img)*args.train_split_coef_b):]
    ...
    for data_source in args.source_list : 
        ...
        a_domain_img_val = a_domain_img[int(len(a_domain_img)*coef_a):]
        a_domain_lbl_val = a_domain_lbl[int(len(a_domain_lbl)*coef_a):]
    ...
    early_stopping = EarlyStopping(patience=15, verbose=True, delta=0.001,path=args.model_path.format(args.training_mode,len(args.source_list), args.data_target, seed))
    ...
    txt = "Seed: {} \n replay {} domaines A {} domaine B {} \n\n".format(seed, args.training_mode,args.source_list, args.data_target)
    f.write(txt)
    f.close()
