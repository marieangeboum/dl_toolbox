...
def main():   
    ...
    #parser.add_argument('--inference_domain',type = str, default = 'austin')
    ...
    train_visu = (args.model_path+"_result_viz").format(args.data_source,args.data_target, args.img_aug)  
    ...
    early_stopping = EarlyStopping(patience=20, verbose=True, delta=0.001,path=args.model_path.format(args.data_source,args.data_target, args.img_aug))
    ...
    if args.data_source == 'austin':        
        ...
        if args.data_target == 'kitsap':
            norm_source_target = transforms.Compose([transforms.Normalize(InriaAustinKitsapDs.stats['mean'], InriaAustinKitsapDs.stats['std'])])
    
    if args.data_source == 'vienna':      
        if args.data_target == 'chicago':
            norm_source_target = transforms.Compose([transforms.Normalize(InriaViennaChicagoDs.stats['mean'], InriaViennaChicagoDs.stats['std'])])
        if args.data_target == 'tyrol-w':
            norm_source_target = transforms.Compose([transforms.Normalize(InriaTyrolViennaDs.stats['mean'], InriaTyrolViennaDs.stats['std'])])
        if args.data_target == 'kitsap':
            norm_source_target = transforms.Compose([transforms.Normalize(InriaKitsapViennaDs.stats['mean'], InriaKitsapViennaDs.stats['std'])])   
    ...
    if args.data_source == 'chicago':
        ...
        if args.data_target == 'austin':
            norm_source_target = transforms.Compose([transforms.Normalize(InriaAustinChicagoDs.stats['mean'], InriaAustinChicagoDs.stats['std'])])
        if args.data_target == 'tyrol-w':
            norm_source_target = transforms.Compose([transforms.Normalize(InriaTyrolChicagoDs.stats['mean'], InriaTyrolChicagoDs.stats['std'])])
        if args.data_target == 'kitsap':
            norm_source_target = transforms.Compose([transforms.Normalize(InriaKitsapChicagoDs.stats['mean'], InriaViennaKitsapChicagoDs.stats['std'])])   
    ...
    if args.data_source == 'kitsap':        
        ...
        if args.data_target == 'austin':
            norm_source_target = transforms.Compose([transforms.Normalize(InriaAustinKitsapDs.stats['mean'], InriaAustinKitsapDs.stats['std'])])
        if args.data_target == 'chicago':
            norm_source_target = transforms.Compose([transforms.Normalize(InriaKitsapChicagoDs.stats['mean'], InriaKitsapChicagoDs.stats['std'])])
        if args.data_target == 'tyrol-w':
            norm_source_target = transforms.Compose([transforms.Normalize(InriaTyrolKitsapDs.stats['mean'], InriaTyrolKitsapDs.stats['std'])]) 
    ...
    a_domain_lbl_train = a_domain_lbl[:int(len(a_domain_lbl)*args.train_split_coef)]
       
    a_domain_img_inf = a_domain_img[int(len(a_domain_img)*args.train_split_coef):]
    a_domain_lbl_inf = a_domain_lbl[int(len(a_domain_lbl)*args.train_split_coef):]
    ...
    b_domain_lbl_train = b_domain_lbl[:int(len(b_domain_lbl)*args.train_split_coef)]
    
    b_domain_img_inf = b_domain_img[int(len(b_domain_img)*args.train_split_coef):]
    b_domain_lbl_inf = b_domain_lbl[int(len(b_domain_lbl)*args.train_split_coef):]
    ...
    b_domain_img_test = b_domain_img_inf[int(len(b_domain_img_inf)*args.test_split_coef):]
    b_domain_lbl_test = b_domain_lbl_inf[int(len(b_domain_lbl_inf)*args.test_split_coef):]
        
    train_domains_imgs = a_domain_img_train + b_domain_img_train
    train_domains_lbls = a_domain_lbl_train + b_domain_lbl_train 
    train_image_paths_list, train_label_paths_list = shuffle(np.array(train_domains_imgs),np.array(train_domains_lbls))
    
    val_domains_imgs = a_domain_img_val + b_domain_img_val
    val_domains_lbls = a_domain_lbl_val + b_domain_lbl_val
    ...
    # test_image_paths_list, test_label_paths_list = shuffle(np.array(test_domains_imgs),np.array(test_domains_lbls))  
    test_b_domain_imgs = b_domain_img_test 
    test_b_domain_lbl  = b_domain_lbl_test
    
    test_list = open("test_{}+{}.txt".format(args.data_source, args.data_target), "w")
    for item in (test_a_domain_imgs,test_a_domain_lbl, test_b_domain_imgs,test_b_domain_lbl ):
        test_list.write("%s\n" % item)
    test_list.close()
