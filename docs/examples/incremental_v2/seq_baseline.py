...
def main():   
    ...
    parser.add_argument('-n', '--sequence_list', nargs='+', default=['chicago', 'kitsap'])
    ...
    print("Check file!")
    step =  0    
    for data_target in args.sequence_list :        
        
        a_domain_img = [item for item in train_imgs if fnmatch.fnmatch(item, os.path.join(args.data_path, 'images/{}*.tif'.format(data_target)))]
        a_domain_lbl = [item for item in train_lbl if fnmatch.fnmatch(item, os.path.join(args.data_path, 'gt/{}*.tif'.format(data_target)))]
        ...
        for training_mode in ("encoder_decoder", "decoder"):
            if step !=0 : 
                ...
                saved_model_path = os.path.join(args.sequence_path.format(seed), 'seq{}_step{}{}'.format(seed, step-1, training_mode))
