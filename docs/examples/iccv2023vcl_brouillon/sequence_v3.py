...
def main():          
    ...
    for domain in sequence_list:
        ...
        f=open(args.sequence_path.format(seed)+"/logfilev3{}.txt".format(seed),"a+")
        txt = "Sequence{} : \n".format(seed)
        ...
        for training_mode in ("model", "decoder"):
            ...
            f.write(txt)
            f.close()
            model_path= os.path.join(args.sequence_path.format(seed), 'inria_{}_{}_step{}{}'.format(args.strategy,seed, step, training_mode))
            if step == 0 :
                idx = step
                if training_mode == 'decoder': 
                    # step += 1
                    continue
                model_path= os.path.join(args.sequence_path.format(seed), 'inria_{}_step{}{}'.format(seed, step, training_mode))
            ...
            if step == 0:
               saved_model_path = os.path.join(args.sequence_path.format(seed), 'pretrainingINRIA/all_seed_3939')
               model.load_state_dict(torch.load(saved_model_path)) 
               model.to(device) 
                      
            if step == 1 :  
                idx = step-1                   
                saved_model_path = os.path.join(args.sequence_path.format(seed), 'inria_{}_step{}{}'.format(seed, int(idx), "model"))
                model.load_state_dict(torch.load(saved_model_path))
                model.to(device) 
            elif step!=0 and step!= 1:
                idx = step-1              
                saved_model_path = os.path.join(args.sequence_path.format(seed), 'inria_{}_{}_step{}{}'.format(args.strategy,seed, int(idx), training_mode))
                model.load_state_dict(torch.load(saved_model_path))
                model.to(device) 
