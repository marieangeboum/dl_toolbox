...
parser.add_argument('--seqlist',  nargs='+', default=[])
...
def main():          
    ...
    idx = step if step ==0 else step-1
    print(sequence_list)
    first_img = []
    # first_lbl = []
    
    for domains in sequence_list[:6]:
        img = glob.glob(os.path.join(args.data_path, '{}/Z*_*/img/IMG_*.tif'.format(domains)))
        # lbl = glob.glob(os.path.join(args.data_path, '{}/Z*_*/msk/MSK_*.tif'.format(domains)))
        # img, lbl = shuffle(np.array(img),np.array(lbl))
        random.shuffle(np.array(img))
        # img = img.tolist()
        # lbl = lbl.tolist()
        
        first_img.append(img)
        # first_lbl.append(lbl)
        
    random.shuffle(first_img)
    
    first_train_imgs = img[:int(len(first_img)*args.train_split_coef)]
    # first_train_lbl  = lbl[:int(len(first_lbl)*args.train_split_coef)]
    
    first_train_imgs = img[:int(len(first_train_imgs)*args.train_split_coef)]
    # first_train_lbl  = lbl[:int(len(first_train_lbl)*args.train_split_coef)]
    
    first_val_imgs = img[int(len(first_train_imgs)*args.train_split_coef):]
    # first_val_lbl  = lbl[int(len(first_train_lbl)*args.train_split_coef):]
    
    first_test_imgs = img[int(len(first_img)*args.train_split_coef):]
    # first_test_lbl  = lbl[int(len(first_lbl)*args.train_split_coef):]
    f=open(args.sequence_path.format(seed)+"/logfilev2_{}.txt".format(seed),"a+")
    txt = "Sequence{} : \n".format(seed)
    
    f.write(txt)
    f.write("Encoder : {}".format(args.encoder))
    f.write("First 5 domains")
    for item_x in (first_test_imgs) :
        f.write(item_x + '\n') 
        
    f.close()
    
    # Train dataset
    first_train_datasets = []
    for img_path in first_train_imgs : 
        ...
        # for img_path, lbl_path in zip( first_train_imgs, first_train_lbl):            
        first_train_datasets.append(FlairDs(image_path = img_path, label_path = lbl_path, fixed_crops = False,
                            tile=Window(col_off=0, row_off=0, width=args.tile_width, height=args.tile_height),
                            crop_size=args.crop_size,        
                            crop_step=args.crop_size,
                            img_aug=args.img_aug))
        
    first_trainset = ConcatDataset(first_train_datasets) 
    
    first_train_sampler = RandomSampler(
        data_source=first_trainset,
        replacement=False,
        num_samples=args.epoch_len)
    
    first_train_dataloader = DataLoader(
        dataset=first_trainset,
        batch_size=args.sup_batch_size,
        sampler=first_train_sampler,
        collate_fn = CustomCollate(batch_aug = args.img_aug),
        num_workers=args.workers)  
    
    # Validation dataset
    first_val_datasets = []
    for img_path in first_val_imgs : 
        img_path_strings = img_path.split('/')
        domain_pattern = img_path_strings[-4]
        img_pattern = img_path_strings[-1].split('_')[-1].strip('.tif')
        lbl_path = glob.glob(os.path.join(args.data_path, '{}/Z*_*/msk/MSK_{}.tif'.format(domain_pattern, img_pattern)))[0]
          
        # for img_path, lbl_path in zip(first_val_imgs, first_val_lbl):             
        first_val_datasets.append(FlairDs(image_path = img_path, label_path = lbl_path, fixed_crops = True,
                            tile=Window(col_off=0, row_off=0,  width=args.tile_width, height=args.tile_height),
                            crop_size=args.crop_size,
                            crop_step=args.crop_size,
                            img_aug=args.img_aug))
         
    first_valset =  ConcatDataset(first_val_datasets)
    
    first_val_dataloader = DataLoader(
        dataset=first_valset,
        shuffle=False,
        batch_size=args.sup_batch_size,
        collate_fn = CustomCollate(batch_aug = args.img_aug),
        num_workers=args.workers) 
    ...
    for domain in sequence_list[6:]:
        ...
        random.shuffle(np.array(domain_img))
        ...
        for img_path in domain_img_train : 
            ...
            next_train_datasets.append(FlairDs(image_path = img_path, label_path = lbl_path, fixed_crops = False,
                                tile=Window(col_off=0, row_off=0, width=args.tile_width, height=args.tile_height),
                                crop_size=args.crop_size,        
                                crop_step=args.crop_size,
                                img_aug=args.img_aug))
            
        next_trainset = ConcatDataset(next_train_datasets) 
        
        next_train_sampler = RandomSampler(
            data_source=next_trainset,
            replacement=False,
            num_samples=args.epoch_len)
        
        next_train_dataloader = DataLoader(
            dataset=next_trainset,
            batch_size=args.sup_batch_size,
            sampler=next_train_sampler,
            collate_fn = CustomCollate(batch_aug = args.img_aug),
            num_workers=args.workers)  
        ...
        for img_path in domain_img_train : 
            ...
            next_val_datasets.append(FlairDs(image_path = img_path, label_path = lbl_path, fixed_crops = True,
                                tile=Window(col_off=0, row_off=0,  width=args.tile_width, height=args.tile_height),
                                crop_size=args.crop_size,
                                crop_step=args.crop_size,
                                img_aug=args.img_aug))
             
        next_valset =  ConcatDataset(next_val_datasets)
        
        next_val_dataloader = DataLoader(
            dataset=next_valset,
            shuffle=False,
            batch_size=args.sup_batch_size,
            collate_fn = CustomCollate(batch_aug = args.img_aug),
        ...
        for training_mode in ("model", "decoder"):
            ...
            txt = "5 domains first {} {} Step {} \n {}\n".format(args.strategy,training_mode,step, sequence_list)
            print(sequence_list)
            f.write(txt)
            f.close()
            model_path= os.path.join(args.sequence_path.format(seed), 'alt_{}_{}_step{}{}'.format(args.strategy,seed, step, training_mode))
            if step == 0 :
                idx = step
                if training_mode == 'decoder': 
                    
                    continue
                model_path= os.path.join(args.sequence_path.format(seed), 'alt_{}_step{}{}'.format(seed, step, training_mode))
            ...
            if step == 1 :    
                idx  = step -1               
                saved_model_path = os.path.join(args.sequence_path.format(seed), 'alt_{}_step{}{}'.format(seed, int(idx), "model"))
                model.load_state_dict(torch.load(saved_model_path))
            elif step!=0 and step!= 1:     
                idx = step-1 
                saved_model_path = os.path.join(args.sequence_path.format(seed), 'alt_{}_{}_step{}{}'.format(args.strategy,seed, int(idx), training_mode))
                model.load_state_dict(torch.load(saved_model_path))
            ...
            for epoch in range(start_epoch, args.max_epochs):
                ...
                acc_sum  = 0.0                    
                time_ep = time.time()
                train_dataloader = first_train_dataloader if step ==0 else next_train_dataloader
                ...
                scheduler.step()
                model.eval()
                val_dataloader = first_val_dataloader if step ==0 else next_val_dataloader   
