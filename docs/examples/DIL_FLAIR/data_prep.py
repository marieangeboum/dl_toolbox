...
parser.add_argument("--data_path", type=str, default ='/data/FLAIR_1/train')
...
def main():  
    ...
    parser_data.add_argument("--data_path", type=str, default ='/data/FLAIR_1/train')
    parser_data.add_argument("--metadata", type=str, default = 'flair-one_metadata.json')
    parser_data.add_argument("--target_camera", type=str, default ='UCE')
    parser_data.add_argument("--target_year", type=str, default ='2020')
    parser_data.add_argument("--target_zone", type=str, default ='UU')
    parser_data.add_argument("--time_slot", nargs=2, type=float, default=(8,11))
    args_data = parser_data.parse_args()
    ...
    target_zone = args_data.target_zone
    target_camera = args_data.target_camera
    target_year = args_data.target_year
    time_slot = args_data.time_slot
    file_path = args_data.metadata
    time_format = '%Hh%M'
    start_time_slot = time(args_data.time_slot[0],0)
    end_time_slot = time(args_data.time_slot[1],0)
    ...
    for domain in domains_list :
        ...
        paths.extend(glob.glob(os.path.join(args_data.data_path, '{}/Z*_{}/img/IMG_*.tif'.format(domain, target_zone))))
    ...
    try:
        ...
        for domain, step in list_of_tuples :
            ...
            train_writer_comment = ("visualisation_"+ model_path)
            ...
            encoder_params = multi_head_model.encoder.parameters()
            decoder_params = multi_head_model.decoder.parameters()
            model_shared_layers_params = list(encoder_params) + list(decoder_params)
            optimizer_shared_layers = SGD(
                model_shared_layers_params,
                lr=args.initial_lr,
                momentum=0.9)
            optimizer_segmentation_heads = SGD(
                multi_head_model.segmentation_heads.parameters(),
                lr=args.initial_lr,
                momentum=0.9)
            
            loss_fn = torch.nn.BCEWithLogitsLoss() 
            scheduler_shared_layers = LambdaLR(optimizer_shared_layers,lr_lambda= lambda_lr, verbose = True)
            scheduler_segmentation_heads  = LambdaLR(optimizer_segmentation_heads ,lr_lambda= lambda_lr, verbose = True)
            ...
            for epoch in range(start_epoch, args.max_epochs):
                ...
                for i, batch in enumerate(train_dataloader):                     
                    ...
                    print(domain_ids)
                    optimizer_shared_layers.zero_grad()                         
                    ...
                    loss.backward()
                    optimizer_shared_layers.step()  # updating parameters                        
                    acc_sum += accuracy(torch.transpose(logits,0,1).reshape(2, -1).t(), 
                                        torch.transpose(target.to(torch.uint8),0,1).reshape(2, -1).t())
                    loss_sum += loss.item()                  
                
                image_segmentation_visualisation.display_batch(train_writer, batch, 5,epoch,prefix='train')      
                ...
                for i, batch in enumerate(val_dataloader):
                    ...
                    acc_sum += accuracy(torch.transpose(output,0,1).reshape(2, -1).t(), 
                                        torch.transpose(target.to(torch.uint8),0,1).reshape(2, -1).t())
                image_segmentation_visualisation.display_batch(train_writer, batch,1, epoch,prefix = 'val')
