...
class FlairDs(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with rasterio.open(self.image_path) as image_file:
            ...
            id_domain = domain_pattern.strip('D').replace('_','')
            id_canal = np.ones(image_rasterio.shape)*int(id_domain)
            new_image = np.dstack(image_rasterio, id_canal, dtype = image_rasterio.dtype)
